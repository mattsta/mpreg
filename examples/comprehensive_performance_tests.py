...
Run with: poetry run python examples/comprehensive_performance_tests.py
"""
            ...
            try:
                async with MPREGClientAPI("ws://127.0.0.1:9001") as client:
                    ...
                    for i in range(concurrent_requests):
                        task = client.call("scale_op", i, locs=frozenset(["scaling"]))
...
class EdgeCaseTestSuite:
    ...
    async def test_error_handling_performance(self):
        ...
        server = MPREGServer(
            MPREGSettings(
                port=9001, name="Error-Server", resources={"error"}, log_level="WARNING"
            )
        )
