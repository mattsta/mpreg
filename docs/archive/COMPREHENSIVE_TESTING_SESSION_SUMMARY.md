...
NOTE
This is a historical testing log from the federation-to-fabric transition.
Treat "federation" references as "fabric" and use `docs/UNIFIED_UNIFICATION_PLAN.md`
for current status and `docs/TEST_PARITY_MATRIX.md` for coverage tracking.
...
- `/Users/matt/repos/mpreg/mpreg/core/port_allocator.py` (Fixed port allocation from 4 to 8 ports)
...
- `mpreg/core/port_allocator.py` - Fixed port allocation (4→8 ports)
