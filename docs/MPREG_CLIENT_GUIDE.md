...
```bash
# Build and run
native build -t mpreg-cache-server .
native run -p 9001:9001 mpreg-cache-server

# Connect client from another container
native run --network host mpreg-cache-server poetry run python mpreg/examples/cache_client_server_demo.py --mode client --url ws://:9001
```
