...
`strong.cft_retry_abort_self_target` for RPC fan-in peers=[self], and
`strong.cft_gcm_retry_abort_clears_residual` for the GCM library surface).
...
(**int**), `last_abort_fail_peers` (**list**), `last_abort_fail_op_id` (**str**),
`residual_ops_hint` (**str**) via `strong_doctor_json_residual_fields`,
