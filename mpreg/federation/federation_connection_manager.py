...
class FederationConnectionManager:
    ...
    def handle_connection_hello(
        ...
        """Handle HELLO message from remote federation node."""
        ...
        if decision.allowed and decision.bridge_config:
            logger.info(
...
def should_allow_cross_federation_hello(
    ...
    """
    Helper function to determine if cross-federation HELLO should be allowed.

    Returns:
        tuple[bool, str]: (allowed, error_message_if_rejected)
    """
    decision = connection_manager.handle_connection_hello(
        remote_cluster_id=remote_cluster_id,
        remote_node_id=remote_node_id,
        remote_url=remote_url,
        local_cluster_id=local_cluster_id,
