"""
Membership and Failure Detection for MPREG Federation.

This module implements advanced membership management and failure detection
for the planet-scale federation system. It provides SWIM-based failure detection
...
This is Phase 3.3 of the Planet-Scale Federation Roadmap.
"""
...
class MembershipProtocol:
    ...
    async def _update_node_membership(
        self, node_id: str, updates: MembershipUpdatePayload | dict[str, Any]
    ) -> None:
        ...
        with self._lock:
            ...
            if isinstance(updates, MembershipUpdatePayload):
                ...
                # Legacy dict handling - set known member info fields
                for key, value in updates.items():
                    setattr(member_info, key, value)
