"""L1 auto_port_bootstrap — OS-assigned ports + peer join without fixed numbers."""
...
async def main() -> None:
    ...
    try:
        ...
        async with MPREGClientAPI(url_b) as client:
            result = await client.call("ping", "auto", locs=frozenset(["api"]))
            peers = await client.list_peers()
        ensure(result == "pong:auto", f"bad result {result!r}")
        ensure(assigned.get("a") and assigned.get("b"), f"capture failed {assigned}")
        ok(f"auto-port bootstrap rpc={result!r} peers={peers!r} assigned={assigned}")
    finally:
