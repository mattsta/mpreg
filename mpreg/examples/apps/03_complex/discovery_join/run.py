            ...
            async def _run(servers: list[MPREGServer]) -> None:
                ...
                with scenario(
                    ...
                    async with MPREGClientAPI(url_a) as client:
                        ra = await client.call(
                            "ping_a", locs=frozenset(["api", "core"])
                        )
                        rb = await client.call(
                            "ping_b", locs=frozenset(["api", "edge"])
                        )
                        rc = await client.call(
                            "ping_c", locs=frozenset(["api", "worker"])
                        )
