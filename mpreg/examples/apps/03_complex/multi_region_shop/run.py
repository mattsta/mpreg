"""L3 multi_region_shop — two-cluster fabric RPC with permissive bridging."""
    ...
    with port_range_context(2, "servers") as ports:
        ...
        async def _run(servers: list[MPREGServer]) -> None:
            ...
            def inventory_lookup(sku: str) -> dict[str, object]:
                return {"sku": sku, "region": "west", "qty": 10}
            ...
            async def _fabric_ready() -> bool:
                # Peer URL and/or function table propagation
            ...
            try:
                await wait_until(
                    _fabric_ready, timeout_s=10.0, what="fabric peer/function visibility"
                )
            except Exception:
                # Soft settle — still attempt client call (tier1 fabric often works
                # after run_with_servers settle alone).
            ...
            ensure("inv" in result and "quote" in result, f"missing keys {result}")
            inv, quote = result["inv"], result["quote"]
            ensure(
                isinstance(inv, dict) and inv.get("region") == "west",
                f"bad inv {inv}",
            )
            ensure(
            ...
            ok(f"multi-region shop inv={inv} quote={quote}")
            step(
