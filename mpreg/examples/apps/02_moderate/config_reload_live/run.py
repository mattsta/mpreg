"""L2 config_reload_live — cache+queue survive process restart via SQLite persistence."""
...
async def _start(port: int, data_dir: Path, name: str) -> tuple[MPREGServer, asyncio.Task[None]]:
...
async def main() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with port_context("servers") as port:
            ...
            try:
                assert server._queue_manager is not None
                assert server._cache_manager is not None
        ...
        with port_context("servers") as port:
            ...
            try:
                ...
                ensure(hit.success and hit.entry is not None, "cache miss after restart")
                ensure(hit.entry.value.get("theme") == "dark", f"bad {hit.entry.value}")
