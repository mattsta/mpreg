...
def test_t26_openapi_both_schemas_present() -> None:
    ...
    strong_caps = (
        ((schemas["StrongMetricsResponse"].get("properties") or {})
         .get("strong") or {})
        .get("properties") or {}
    ).get("capabilities") or {}
    audit_caps = (
        ((schemas["SharedAuditMetricsResponse"].get("properties") or {})
         .get("shared_audit") or {})
        .get("properties") or {}
    ).get("capabilities") or {}
    assert (strong_caps.get("properties") or {}).get("get_quorum", {}).get(
        "enum"
    ) == [False]
    assert (audit_caps.get("properties") or {}).get("siem", {}).get("enum") == [
        False
    ]
...
def test_t26_operate_mentions_ci_core_and_caps() -> None:
    ...
    operate = (root / "docs/examples-curriculum/OPERATE.md").read_text(
        encoding="utf-8"
    )
