...
This will trace:
1. What advertised_urls are being sent in STATUS messages
...
async def debug_peer_discovery_mechanism():
    ...
    with TestPortManager() as port_manager:
        ...
        if not discovery_working:
            ...
            print(
                "  1. STATUS messages contain advertised_urls but only share function metadata"
            )
            print(
                "  2. Peers are added to cluster.peers_info but no active connections are made to advertised_urls"
            )
            print(
                "  3. _manage_peer_connections should connect to all peers_info entries"
            )
            print(
